#!/usr/bin/env python
"""
Build XGBoost forecasting models for top items - simplified version.
"""
import sys
...
def train_xgboost_forecaster(item_data: pd.Series, item_id: str, horizon: int = 14) -> Dict:
    """Train XGBoost forecaster for a single item - simplified version."""
...
def compare_with_baselines(xgb_results: Dict, baseline_results_path: Path) -> pd.DataFrame:
    ...
    for item_id, xgb_data in xgb_results.items():
        ...
        item_baselines = baseline_df[baseline_df['item_id'] == item_id_int]
...
def main():
    ...
    print("XGBOOST MODEL TRAINING (SIMPLIFIED)")
    print("="*50)
